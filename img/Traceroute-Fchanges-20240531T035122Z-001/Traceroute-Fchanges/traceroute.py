...
def send_udp(ttl, port, destination_address,timeout=None):
    ssnd = socket_creation.create_udp_send_socket(ttl, isIPV6)
    udp_packet = create_udp_packet(port)
    ssnd.sendto(udp_packet, (destination_address, port))
    return ssnd

def send_icmp(ttl, destination_address, timeout):
...
def send_tcp(ttl, port, destination_address, timeout=None):
    ssnd = socket_creation.create_tcp_send_socket(ttl, isIPV6)
    try:
        if ssnd:
    ...
    return ssnd
def traceroute(destination_address, isIPV6, max_hops=60, timeout=3, max_rejections=15, update_output=None, add_router=None, with_location=False, protocol=None):
    ttl = 1
    rejections = 0
    addresses = []
    port = 33434
    ...
    while ttl < max_hops:
        srec = socket_creation.create_icmp_receive_socket(port, timeout, DEVICE_NAME, isIPV6)
        
        ssend = send_function(ttl, destination_address, timeout)
...
def start(website_address, ttl=60, timeout=3, max_rejections=15, update_output=None, add_router=None, with_geoinfo=False, protocols=[]):
    website, website_address = get_website(isIPV6, website_address)

    if update_output:
        update_output(f"Destino: {website} ({website_address})")

    for prot in protocols:
        traceroute(website_address, isIPV6, max_hops=ttl, timeout=timeout, max_rejections=max_rejections,
